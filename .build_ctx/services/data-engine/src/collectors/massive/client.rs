...
impl MassiveClient {
    ...
    pub async fn get_aggregates(
        ...
        let resp = self.client
            .get(&url)
            .query(&[("apiKey", self.api_key.as_str()), ("adjusted", "true"), ("sort", "asc"), ("limit", "50000")])
            .send()
            .await
        ...
        let data: AggregatesResponse = resp.json().await.context("Failed to parse Massive aggregates")?;
        
        if data.status != "OK" {
             // Sometimes status is "DELAYED" which is fine, but "ERROR" is bad.
             if data.status == "ERROR" {
                 return Err(anyhow::anyhow!("Massive API returned status: {}", data.status));
             }
        }
