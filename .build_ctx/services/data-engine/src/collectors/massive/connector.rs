...
use crate::models::{AssetType, DataSourceType, StandardMarketData, MarketDataType};
use crate::error::{Result, DataError};
use super::client::{MassiveClient, AggregateResult};
...
impl MassiveConnector {
    ...
    pub async fn fetch_history_candles(
        ...
        let aggregates = self.client.get_aggregates(ticker, multiplier, timespan, from, to)
            .await
    ...
    fn map_aggregate(&self, ticker: &str, agg: AggregateResult) -> StandardMarketData {
        ...
        StandardMarketData {
            ...
            asset_type: if ticker.starts_with("X:") { AssetType::Crypto } else if ticker.starts_with("O:") { AssetType::Option } else { AssetType::Stock },
...
impl DataSourceConnector for MassiveConnector {
    ...
    fn supported_assets(&self) -> Vec<AssetType> {
        vec![AssetType::Spot, AssetType::Stock, AssetType::Crypto, AssetType::Option]
    }
