...
use crate::models::{StandardMarketData, DataSourceType, MarketDataType, AssetType};
use chrono::Utc;
...
impl HeliusConnector {
    ...
    pub async fn connect(&self) -> Result<tokio::sync::mpsc::Receiver<StandardMarketData>, Box<dyn Error + Send + Sync>> {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let ws_url = format!("{}/?api-key={}", 
                    config.ws_url.trim_end_matches('/').replace("https://", "wss://"),
                    config.api_key
                );
                info!("Connecting to Helius WS: {}", ws_url.replace(&config.api_key, "***"));
                ...
                match connect_async(&ws_url).await {
                    Ok((mut ws_stream, _)) => {
                        ...
                        info!("📡 Subscribed to Helius slot updates");
                        
                        while let Some(msg) = ws_stream.next().await {
                             match msg {
                                 Ok(tokio_tungstenite::tungstenite::Message::Text(text)) => {
                                     // Log first few messages for debugging
                                     info!("Helius: {}", &text[..text.len().min(200)]);
                                 },
