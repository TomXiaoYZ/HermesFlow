...
impl TokenDiscoveryTask {
    ...
    pub async fn run(&self) {
        ...
        for page in 0..num_pages {
            ...
            match self.birdeye_client.get_trending_tokens(page_size, offset).await {
                Ok(mut tokens) => {
                    info!("📄 Page {}/{}: {} tokens (offset={})", 
                          page + 1, num_pages, tokens.len(), offset);
                ...
                Err(e) => {
                    warn!("Failed to fetch page {} (offset={}): {}", page + 1, offset, e);
        ...
        info!("Fetched {} total trending tokens from Birdeye", all_trending.len());
        ...
        info!("Filtered {} tokens (liq >${}, fdv ${}-${})", 
              filtered_tokens.len(), 
              self.min_liquidity_usd,
              self.min_fdv,
              if self.max_fdv.is_infinite() { "inf".to_string() } else { format!("{}", self.max_fdv) }
        );
        ...
        let tokens_to_upsert: Vec<ActiveToken> = filtered_tokens.iter().map(|t| {
            ActiveToken {
        ...
        match self.token_repo.upsert_tokens(tokens_to_upsert).await {
            Ok(_) => {
                info!("✅ Successfully upserted {} active tokens to DB", filtered_tokens.len());
            }
            Err(e) => {
