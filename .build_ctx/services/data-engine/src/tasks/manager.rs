...
impl TaskManager {
    pub async fn new(config: AppConfig, repos: Arc<PostgresRepositories>) -> Result<Self, Box<dyn std::error::Error>> {
                    ...
                    Ok(symbols) => {
                        ...
                        if let Some(massive_cfg) = config.massive {
                            ...
                            for symbol in symbols {
                                match connector.fetch_history_candles(&symbol, 1, "day", &date_str, &date_str).await {
                                    Ok(candles) => {
                                        for data in candles {
                                             let meta_value = serde_json::from_str::<Value>(&data.raw_data).ok();
                                             let open = if let Some(json) = &meta_value {
                                                 json.get("open").and_then(|v| v.as_f64()).map(|f| Decimal::from_f64_retain(f).unwrap_or_default()).unwrap_or(data.price)
                                             } else { data.price };
                                             ...
                                             if let Err(e) = repos.market_data.insert_candle(&candle).await {
                                                 error!("EOD: Failed to insert {} for {}: {}", symbol, date_str, e);
                                             }
                                        }
                                    }
                                    Err(e) => error!("EOD: Failed to fetch {} for {}: {}", symbol, date_str, e),
                                }
                                // Rate limit sleep
    ...
    pub async fn trigger_backfill(&self, symbol: String, from: String, to: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
        info!("Triggering manual backfill for {} from {} to {:?}", symbol, from, to);
        ...
        tokio::spawn(async move {
            ...
            use crate::collectors::{MassiveConnector, BirdeyeConnector};
            ...
            if is_contract {
                if let Some(birdeye_cfg) = config.birdeye {
                     ...
                     let from_ts = Utc::now().checked_sub_signed(chrono::Duration::days(30)).unwrap().timestamp(); // Default 30 days? API needs explicit.

                     match connector.fetch_history_candles(&symbol, "1H", from_ts, to_ts).await {
                         Ok(candles) => {
                             ...
                             for candle in candles {
                                 ...
                                 if let Err(e) = repos_clone_for_task.market_data.insert_candle(&candle).await {
                                     error!("Failed to insert Birdeye candle for {}: {}", symbol, e);
                                 } else {
                                     success_count += 1;
                                 }
                             }
                             info!("Birdeye Backfill complete for {}. Inserted {}/{}", symbol, success_count, total);
                         }
                ...
                if let Some(massive_cfg) = config.massive {
                     ...
                     match connector.fetch_history_candles(&symbol, 1, "day", &from, &to_date).await {
                         Ok(candles) => {
                             ...
                             info!("Backfill complete for {}. Inserted {}/{}", symbol, success_count, total_candles);
                         }
                         Err(e) => {
