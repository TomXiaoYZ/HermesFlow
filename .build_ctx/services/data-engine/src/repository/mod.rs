...
use crate::models::{StandardMarketData, Candle, SocialData, PredictionMarket, MarketOutcome, Order, Trade};
...
pub trait PredictionRepository: Send + Sync {
    ...
    async fn insert_outcome(&self, market_id: &str, outcome: &MarketOutcome) -> Result<(), DataEngineError>;
}
pub mod token;
pub use token::{TokenRepository, ActiveToken};
