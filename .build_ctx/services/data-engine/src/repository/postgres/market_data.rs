...
impl MarketDataRepository for PostgresMarketDataRepository {
    async fn insert_snapshot(&self, data: &StandardMarketData) -> Result<(), DataEngineError> {
        let ts = chrono::DateTime::from_timestamp(data.timestamp / 1000, ((data.timestamp % 1000) * 1_000_000) as u32).unwrap_or(chrono::Utc::now());
        tracing::info!("Inserting snapshot for {}: price={}, ts={:?}", data.symbol, data.price, ts);
    ...
    async fn get_active_symbols(&self) -> Result<Vec<String>, DataEngineError> {
        let rows = sqlx::query(
            r#"SELECT DISTINCT symbol FROM mkt_equity_snapshots"#
        )
        .fetch_all(&self.pool)
        .await
        .map_err(|e| DataEngineError::DatabaseError(format!("Failed to fetch active symbols: {}", e)))?;
