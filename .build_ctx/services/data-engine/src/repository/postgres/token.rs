...
use crate::repository::token::{TokenRepository, ActiveToken};
...
impl TokenRepository for PostgresTokenRepository {
    async fn get_active_addresses(&self) -> Result<Vec<String>, DataEngineError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| DataEngineError::DatabaseError(format!("Failed to fetch active addresses: {}", e)))?;
    ...
    async fn get_active_tokens(&self) -> Result<Vec<ActiveToken>, DataEngineError> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| DataEngineError::DatabaseError(format!("Failed to fetch active tokens: {}", e)))?;
    ...
    async fn deactivate_stale(&self, hours: i64) -> Result<usize, DataEngineError> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| DataEngineError::DatabaseError(format!("Failed to deactivate stale tokens: {}", e)))?;
