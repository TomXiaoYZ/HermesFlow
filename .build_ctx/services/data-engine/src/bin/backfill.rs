use data_engine::{
    ...
    repository::{MarketDataRepository, postgres::PostgresRepositories},
    models::{StandardMarketData, MarketDataType, Candle},
};
use std::sync::Arc;
use clap::Parser;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Connecting to database at {}:{}...", config.postgres.host, config.postgres.port);
    println!("DEBUG: Postgres User: {}, Password len: {}", config.postgres.username, config.postgres.password.len());
    ...
    if args.source == "massive" {
        ...
        info!("Initializing Massive Connector for {} (Rate Limit: 5/min default)", args.symbol);
        ...
        let candles = connector.fetch_history_candles(
            &args.symbol, 
            args.multiplier, 
            &args.timespan, 
            &args.from, 
        ...
        for point in candles {
            ...
            let time = chrono::DateTime::from_timestamp(seconds, nsec)
            ...
            let candle = Candle {
                ...
                resolution: format!("{}{}", args.multiplier, args.timespan.chars().next().unwrap_or('?')), // e.g. "1d"
                open: open_price,
        ...
        info!("Backfill completed. Processed: {}, Upserted: {}", total_count, upserted);
