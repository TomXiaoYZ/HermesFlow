...
impl DataCollector for BybitCollector {
    ...
    async fn start(&mut self, tx: tokio::sync::mpsc::Sender<(MarketData, DataQuality)>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("bybit", "websocket", true);
    ...
    async fn stop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("bybit", "websocket", false);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bybit_collector() {
        // 创建收集器
        let mut collector = BybitCollector::new();
        ...
        // 检查订阅状态
        let status = collector.get_status().await.unwrap();
        assert!(status.subscribed_topics.contains(&"tickers.BTCUSDT".to_string()));
