...
#[async_trait]
impl DataCollector for GateioCollector {
    ...
    async fn start(&mut self, tx: tokio::sync::mpsc::Sender<(MarketData, DataQuality)>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("gateio", "websocket", true);
    ...
    async fn stop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("gateio", "websocket", false);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_gateio_collector() {
        // 创建收集器
        let mut collector = GateioCollector::new();
        ...
        // 订阅主题
        let topics = vec!["spot.tickers".to_string()];
        assert!(collector.subscribe(topics.clone()).await.is_ok());

        // 检查订阅状态
        let status = collector.get_status().await.unwrap();
        assert!(status.subscribed_topics.contains(&"spot.tickers".to_string()));
