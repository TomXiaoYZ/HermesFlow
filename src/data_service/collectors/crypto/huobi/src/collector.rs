...
#[async_trait]
impl DataCollector for HuobiCollector {
    ...
    async fn start(&mut self, tx: tokio::sync::mpsc::Sender<(MarketData, DataQuality)>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("huobi", "websocket", true);
    ...
    async fn stop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("huobi", "websocket", false);
...
mod tests {
    ...
    async fn test_huobi_collector() {
        // 创建收集器
        let mut collector = HuobiCollector::new();
        ...
        // 订阅主题
        let topics = vec!["market.btcusdt.ticker".to_string()];
        assert!(collector.subscribe(topics.clone()).await.is_ok());

        // 检查订阅状态
        let status = collector.get_status().await.unwrap();
        assert!(status.subscribed_topics.contains(&"market.btcusdt.ticker".to_string()));
