...
impl DataProcessor for OkxProcessor {
    ...
    async fn validate(&self, data: &MarketData) -> Result<DataQuality, Box<dyn std::error::Error>> {
        ...
        // 更新监控指标
        metrics::update_data_quality("okx", "market_data", quality_score);
...
mod tests {
    ...
    async fn test_process_trade() {
        let processor = OkxProcessor::new(HashMap::new());
    ...
    async fn test_symbol_normalization() {
        ...
        assert_eq!(processor.normalize_symbol("BTC-USDT"), "BTCUSDT");
        assert_eq!(processor.normalize_symbol("ETH-USDT"), "ETHUSDT");
        assert_eq!(processor.normalize_symbol("btc-usdt"), "BTCUSDT");
    }
} 