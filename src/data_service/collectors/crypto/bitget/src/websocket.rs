...
impl BitgetWebsocketClient {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        let url = Url::parse(&self.config.ws_base_url)
            .map_err(BitgetError::UrlParseError)?;

        let (ws_stream, _) = connect_async(url).await
            .map_err(BitgetError::WebSocketError)?;
    ...
    async fn login(&mut self) -> Result<()> {
        ...
        let api_key = self.config.api_key.as_ref()
            .ok_or_else(|| BitgetError::AuthenticationError("Missing API key".to_string()))?;
        ...
        if let Some(response) = self.receive().await? {
            if response.event == "login" {
                ...
                Err(BitgetError::AuthenticationError("Login failed".to_string()))
            }
        } else {
            Err(BitgetError::AuthenticationError("No login response".to_string()))
        }
    }
    ...
    pub async fn send(&mut self, message: &str) -> Result<()> {
        if let Some(stream) = &mut self.stream {
            ...
            Err(BitgetError::NetworkError("WebSocket not connected".to_string()))
        }
    }
        ...
        if let Some(stream) = &mut self.stream {
            match stream.next().await {
                ...
                Some(Ok(Message::Close(_))) => {
                    Err(BitgetError::NetworkError("WebSocket closed by server".to_string()))
                }
                Some(Err(e)) => {
                    Err(BitgetError::WebSocketError(e))
                }
                _ => Ok(None),
            }
        } else {
