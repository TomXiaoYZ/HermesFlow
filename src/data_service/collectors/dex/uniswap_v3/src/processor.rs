...
impl UniswapV3Processor {
    ...
    /// 处理Graph API数据
    pub async fn process_graph_data(&self, data: &Value) -> Result<Option<MarketData>, UniswapV3Error> {
        if let Some(pool_data) = data.get("pool") {
            ...
            let token0_address = pool_data["token0"]["id"].as_str()
                .ok_or_else(|| UniswapV3Error::ParseError("Missing token0 address".to_string()))?;
            let token1_address = pool_data["token1"]["id"].as_str()
                .ok_or_else(|| UniswapV3Error::ParseError("Missing token1 address".to_string()))?;
            ...
            let market_data = MarketData {
                ...
                data_type: MarketDataType::LiquidityEvent(LiquidityEvent {
                    ...
                    token0_amount: Decimal::from_str_exact(pool_data["token0Price"].as_str()
                    ...
                    token1_amount: Decimal::from_str_exact(pool_data["token1Price"].as_str()
                    ...
                    liquidity: Decimal::from_str_exact(pool_data["liquidity"].as_str()
                    ...
                    sqrt_price_x96: Decimal::from_str_exact(pool_data["sqrtPrice"].as_str()
                    ...
                    tick: pool_data["tick"].as_i64()
                        .ok_or_else(|| UniswapV3Error::ParseError("Missing tick".to_string()))?,
                    fee_tier: pool_data["feeTier"].as_u64()
    ...
    /// 处理合约事件数据
    pub async fn process_event_data(&self, event: &Value) -> Result<Option<MarketData>, UniswapV3Error> {
    ...
    /// 处理Swap事件
    fn process_swap_event(&self, event: &Value) -> Result<Option<MarketData>, UniswapV3Error> {
        let data = event["data"].as_object()
            .ok_or_else(|| UniswapV3Error::ParseError("Missing event data".to_string()))?;

        let pool_address = event["address"].as_str()
            .ok_or_else(|| UniswapV3Error::ParseError("Missing pool address".to_string()))?;
        ...
        let market_data = MarketData {
            ...
            timestamp: DateTime::from_timestamp_millis(
                event["timestamp"].as_i64()
                    .ok_or_else(|| UniswapV3Error::ParseError("Missing timestamp".to_string()))?)
                .ok_or_else(|| UniswapV3Error::ParseError("Invalid timestamp".to_string()))?
                .with_timezone(&Utc),
            data_type: MarketDataType::Trade(vec![Trade {
                id: event["transactionHash"].as_str()
                    .ok_or_else(|| UniswapV3Error::ParseError("Missing transaction hash".to_string()))?
                    .to_string(),
                price: Decimal::from_str_exact(data["price"].as_str()
                    .ok_or_else(|| UniswapV3Error::ParseError("Missing price".to_string()))?)
                    .map_err(|e| UniswapV3Error::ParseError(format!("Invalid price: {}", e)))?,
                ...
                timestamp: Utc::now(),
                side: if data["zeroForOne"].as_bool().unwrap_or(false) { Side::Sell } else { Side::Buy },
            }]),
            quality: DataQuality::Real,
        };
    ...
    /// 处理Mint事件
    fn process_mint_event(&self, event: &Value) -> Result<Option<MarketData>, UniswapV3Error> {
        let data = event["data"].as_object()
            .ok_or_else(|| UniswapV3Error::ParseError("Missing event data".to_string()))?;
    ...
    /// 处理Burn事件
    fn process_burn_event(&self, event: &Value) -> Result<Option<MarketData>, UniswapV3Error> {
        let data = event["data"].as_object()
            .ok_or_else(|| UniswapV3Error::ParseError("Missing event data".to_string()))?;
...
#[async_trait]
impl DataProcessor for UniswapV3Processor {
    ...
    async fn validate(&self, data: &MarketData) -> Result<DataQuality, Box<dyn std::error::Error>> {
        ...
        // 更新监控指标
        metrics::update_data_quality("uniswap_v3", "market_data", quality_score);
...
mod tests {
    ...
    async fn test_process_swap() {
        let processor = UniswapV3Processor::new(HashMap::new());
