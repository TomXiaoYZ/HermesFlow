...
#[async_trait]
impl DataCollector for BalancerCollector {
    ...
    async fn start(&mut self, tx: tokio::sync::mpsc::Sender<(MarketData, DataQuality)>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("balancer", "contract", true);
    ...
    async fn stop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        metrics::update_connection_status("balancer", "contract", false);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_balancer_collector() {
        // 创建收集器
        let mut collector = BalancerCollector::new();

        // 创建配置
        let mut connection = std::collections::HashMap::new();
        connection.insert(
            "graph_endpoint".to_string(),
            "https://api.thegraph.com/subgraphs/name/balancer-labs/balancer-v2".to_string(),
        );
        connection.insert(
