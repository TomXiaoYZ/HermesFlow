...
impl HistoricalSyncTask {
    ...
    pub async fn run(&self) {
        ...
        info!(
            "Starting Historical Data Backfill for {} symbols...",
            symbols.len()
        );
        ...
        for symbol in &symbols {
            ...
            while attempts < max_attempts {
                match self
                    .birdeye_client
                    .get_history(symbol, start_time, end_time, "1h")
                    .await
                {
                    Ok(items) => {
                        let count = items.len();
                        info!(
                            "Fetched {} candles for {}. Inserting into DB...",
                            count, symbol
                        );
                    ...
                    Err(e) => {
                        attempts += 1;
                        warn!(
                            "Failed to fetch history for {} (Attempt {}/{}): {}",
