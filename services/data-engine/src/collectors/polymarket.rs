...
impl PolymarketCollector {
        ...
        loop {
            ...
            for gamma_market in &markets {
                match self.upsert_gamma_market(gamma_market).await {
                    ...
                    Err(e) => {
                        let id = gamma_market.condition_id.as_deref().unwrap_or("unknown");
    ...
    /// Refresh prices for active markets (lighter operation with pagination)
    pub async fn refresh_prices(&self) -> std::result::Result<(), String> {
        ...
        loop {
            ...
            let markets = match self.fetch_markets_page(&url).await {
                ...
                Err(e) => {
                    error!(
                        "Polymarket: Price refresh failed to fetch page at offset {}: {}",
                        offset, e
                    );
                    break;
                }
            ...
            // Shorter delay than discover since refresh is more time-sensitive
            tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
        }

        info!(
            "Polymarket: Price refresh complete. Updated {} markets.",
            total_updated
        );
        Ok(())
    }

    /// Fetch a single page of markets from the Gamma API with retry logic
    async fn fetch_markets_page(&self, url: &str) -> std::result::Result<Vec<GammaMarket>, String> {
            ...
            || {
                ...
                async move {
                    let response = client
                        ...
                        .await
                        .map_err(|e| {
                            DataError::NetworkError(format!("HTTP request failed: {}", e))
                        })?;
                    ...
                    let markets: Vec<GammaMarket> = response.json().await.map_err(|e| {
                        DataError::NetworkError(format!(
                            "Failed to parse Gamma API response: {}",
                            e
                        ))
                    })?;
    ...
    async fn upsert_gamma_market(&self, gamma: &GammaMarket) -> std::result::Result<(), String> {
        let market_id = gamma
            .condition_id
            .as_deref()
        ...
        let end_date = gamma.end_date.as_deref().and_then(parse_datetime);
...
fn parse_outcomes_and_prices(
    ...
    let total_volume: Option<Decimal> = volume_str.and_then(|s| s.parse::<Decimal>().ok());

    outcome_names
        .into_iter()
        .enumerate()
        .map(|(i, name)| {
            let raw_price = prices.get(i).copied().unwrap_or(Decimal::ZERO);
            let price = if raw_price < Decimal::ZERO || raw_price > Decimal::ONE {
                warn!(
                    "Polymarket: Outcome '{}' has out-of-range price {} (expected 0..=1), clamping",
                    name, raw_price
                );
                raw_price.max(Decimal::ZERO).min(Decimal::ONE)
            } else {
                raw_price
            };
            MarketOutcome {
...
mod tests {
    ...
    fn test_parse_outcomes_and_prices() {
        ...
        assert_eq!(outcomes[0].volume, Some(dec!(1000000)));
    }
    ...
    #[test]
    fn test_parse_outcomes_clamps_negative_price() {
        let outcomes =
            parse_outcomes_and_prices(Some(r#"["Yes","No"]"#), Some(r#"["-0.1","1.1"]"#), None);
        assert_eq!(outcomes.len(), 2);
        assert_eq!(outcomes[0].price, Decimal::ZERO);
        assert_eq!(outcomes[1].price, Decimal::ONE);
    }

    #[test]
    fn test_parse_outcomes_valid_boundary_prices() {
        let outcomes =
            parse_outcomes_and_prices(Some(r#"["Yes","No"]"#), Some(r#"["0","1"]"#), None);
        assert_eq!(outcomes.len(), 2);
        assert_eq!(outcomes[0].price, Decimal::ZERO);
        assert_eq!(outcomes[1].price, Decimal::ONE);
    }
