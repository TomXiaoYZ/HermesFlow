...
use uuid::Uuid;
use chrono::Utc;
use crate::models::{Order, trading::OrderRequest};
...
pub async fn place_order(
    ...
    let trader = state
        .ibkr_trader
        .as_ref()
        .ok_or((StatusCode::SERVICE_UNAVAILABLE, "IBKR Trading not enabled".to_string()))?;
    ...
    let ib_order_id = trader.place_order(payload.clone()).await.map_err(|e| {
        ...
        (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to place order: {}", e))
    })?;
    ...
    let saved_id = state
        ...
        .map_err(|e| {
            ...
            (StatusCode::INTERNAL_SERVER_ERROR, "Order placed but storage failed".to_string())
        })?;

    info!("Order placed successfully: {} (IB: {})", saved_id, ib_order_id);
...
pub async fn cancel_order(
    ...
    trader.cancel_order(ib_order_id).await.map_err(|e| {
        ...
        (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to cancel order: {}", e))
    })?;
...
pub async fn get_positions(
    ...
    let positions = trader.get_positions().await.map_err(|e| {
        ...
        (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to get positions: {}", e))
    })?;
