...
enum Commands {
    ...
    Buy {
        ...
        #[arg(short, long, default_value = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v")]
        token: String,
...
async fn main() -> anyhow::Result<()> {
    ...
    let priv_key = env::var("SOLANA_PRIVATE_KEY").map_err(|_| {
        anyhow::anyhow!("SOLANA_PRIVATE_KEY environment variable not set. Please set it to run live tests.")
    })?;
    ...
    match cli.command {
        ...
        Commands::Buy { token, amount, slippage } => {
            info!("--- Starting LIVE BUY ---");
            info!("Target: swap {} SOL -> {}", amount, token);
            
            match trader.buy_raydium_experimental(&token, amount, slippage).await {
                Ok(sig) => {
                ...
                Err(e) => {
                    error!("❌ FAILED: {:?}", e);
                    if let Some(client_error) = e.downcast_ref::<solana_client::client_error::ClientError>() {
                        if let solana_client::client_error::ClientErrorKind::RpcError(rpc_err) = &client_error.kind {
        ...
        Commands::Sell { token, percentage, slippage } => {
