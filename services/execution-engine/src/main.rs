...
async fn main() -> anyhow::Result<()> {
    ...
    let priv_key = env::var("SOLANA_PRIVATE_KEY")
        ...
        .trim()
        .to_string(); // Trim whitespace just in case

    // DEBUG: Inspect Key
    info!("Key loaded. Length: {}", priv_key.len());
    if priv_key.len() > 10 {
        info!("Key start: {}..., Key end: ...{}", &priv_key[0..5], &priv_key[priv_key.len()-5..]);
    } else {
        info!("Key appears too short!");
    }
    ...
    if let Some(trader) = solana {
        ...
        tokio::spawn(async move {
            ...
            let mut con = client
                .get_connection()
                .expect("Failed to connect to Redis for Portfolio Sync");
